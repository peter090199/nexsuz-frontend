...
//   styleUrls: ['./job-posting-ui.component.scss']
// })
...
import { MatStepper } from '@angular/material/stepper';
import { Router } from '@angular/router';
import { SharedRoutinesService, Currency } from 'src/app/services/Function/shared-routines.service';
import { NotificationsService } from 'src/app/services/Global/notifications.service';
import { JobPostingService } from 'src/app/services/Jobs/job-posting.service';
...
export class JobPostingUIComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currencies = this.sharedRoutineService.getCurrencies();
    this.selectedCurrency = this.sharedRoutineService.getDefaultCurrency().code;
    ...
    if (this.data?.job_id) {
      this.btnSave = "Update";
      this.selectedCurrency = this.data.currency || this.sharedRoutineService.getDefaultCurrency().code;
      this.displayFormData();
    }
  ...
  getCurrencySymbol(): string {
    return this.sharedRoutineService.getCurrencySymbol(this.selectedCurrency);
  }
  ...
  initForms(): void {
    ...
    this.jobForm = this.fb.group({
      ...
      job_about: ['', Validators.required],
      location: ['', Validators.required],
      benefits: ['', [Validators.required, Validators.maxLength(255)]],
      salaryRange: this.fb.group({
        currency: ['PHP'],        // store the code, not the symbol
        minSalary: [null, [Validators.required, Validators.min(0)]],
        maxSalary: [null, [Validators.required, Validators.min(0)]],
      })
    });
  ...
  displayFormData(): void {
    ...
    // Patch job fields (salaryRange patched separately since it's a nested group)
    this.jobForm.patchValue({
      job_name: this.data.job_name,
      job_position: this.data.job_position,
      job_description: this.data.job_description,
      ...
      benefits: this.data.benefits,
      salaryRange: {
        selectedCurrency: this.data.currency ?? this.selectedCurrency,
        minSalary: this.data.min_salary ?? this.data.minSalary ?? null,
        maxSalary: this.data.max_salary ?? this.data.maxSalary ?? null,
      }
    });
  ...
  onSubmit(): void {
    if (this.jobForm.invalid || this.companyForm.invalid || this.appliedQuestionForm.invalid) {
      ...
      this.appliedQuestionForm.markAllAsTouched();
      this.notificationService.toastrError("Please complete all required fields.");
      return;
    }
    ...
    // Pull salaryRange out on its own so it never gets stringified as [object Object]
    const { salaryRange, ...jobFormRest } = this.jobForm.value;

    const payload: any = {
      ...jobFormRest,
      ...this.companyForm.value,
      currency: salaryRange?.currency,
      min_salary: salaryRange?.minSalary,
      max_salary: salaryRange?.maxSalary,
    };
    ...
    questions.forEach((q: any, index: number) => {
      formData.append(`question_text[${index}]`, q.question_text ?? '');
      formData.append(`answer_type[${index}]`, q.answer_type ?? '');
      formData.append(`correct_answer[${index}]`, q.correct_answer ?? '');
      formData.append(`user_answer[${index}]`, q.user_answer ?? '');
    });
